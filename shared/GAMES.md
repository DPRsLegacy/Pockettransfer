...
Clients only mount those title IDs and upload `main`. PKHeX identifies the SAV variant.

## Destination conversion (Home-like)

`EntityConverter.IsConvertibleToFormat` plus destination `MaxSpeciesID`:

- Forward (6/7 → 8 → 9) is allowed when PKHeX has a route.
- Backward (9 → 7, 9 → 6) is rejected (`NoTransferRoute`).
